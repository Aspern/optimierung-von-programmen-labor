...
namespace opl {
    ...
    size_t minimum(const std::array<T, SIZE> &a, size_t left) {
        size_t min = left;
     ...
     */
    template<typename T, size_t SIZE>
    size_t minimumOptimized(const std::array<T, SIZE> &a, size_t left) {
        const size_t nElementsInCacheLine = (opl::constant::cache_line_size / sizeof(T));
        size_t min = left;

        for (size_t i = left; i < SIZE; i += nElementsInCacheLine)
     ...
     */
    template<typename T, size_t SIZE>
    size_t minimumWithPrefetch(const std::array<T, SIZE> &a, size_t left) {
        const size_t nElementsInCacheLine = (opl::constant::cache_line_size / sizeof(T));
        size_t min = left;
